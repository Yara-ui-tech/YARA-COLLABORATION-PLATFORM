...
export default function EducatorPortal() {
      ...
      try {
        ...
        if (emailToUse) {
          ...
          if (isMounted && reg) {
            ...
            // User is not registered for this event — strictly nullify state
  ...
  const isApproved = Boolean(registration?.approval_status === 'approved');
  const isCertUnlocked = Boolean(registration?.certificate_unlocked && registration?.approval_status === 'approved');
  const isAccessGranted = Boolean(registration?.payment_status === 'verified' && registration?.approval_status === 'approved');
        ...
        <div className="relative z-10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8">
          <div className="space-y-4 max-w-2xl">
            ...
            <div className="flex flex-wrap items-center gap-4 pt-2 text-xs font-semibold text-slate-300">
              ...
              <div className="flex items-center gap-2 bg-white/5 border border-white/10 px-3.5 py-2 rounded-xl">
                <ShieldCheck className={`w-4 h-4 ${isRegistered ? 'text-emerald-400' : 'text-amber-400'}`} />
                <span>{isRegistered ? (isCertUnlocked ? 'Role: Certified Educator' : 'Role: Registered Educator (Pending Approval)') : 'Role: Educator (Not Registered)'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="w-full lg:w-auto flex flex-col sm:flex-row lg:flex-col gap-3 shrink-0">
            {!isRegistered ? (
              ...
              <button
                ...
                className="px-6 py-3.5 rounded-2xl bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 font-bold text-sm transition flex items-center justify-center gap-2.5 shadow-xl shadow-amber-500/20 cursor-pointer"
              >
                <Award className="w-5 h-5 text-slate-950" />
                <span>View Official Certificate</span>
              </button>
            ) : (
              <div className="px-6 py-3.5 rounded-2xl bg-white/10 text-amber-300 border border-amber-400/30 font-bold text-xs transition flex items-center justify-center gap-2 text-center">
                <Lock className="w-4 h-4 text-amber-400 shrink-0" />
                <span>Certificate Locked (Awaiting Admin Approval)</span>
              </div>
            )}

            {isRegistered && receiptData && (
              <button
                onClick={() => {
                  setShowReceiptModal(true);
                }}
                className="px-6 py-3.5 rounded-2xl bg-white/10 hover:bg-white/15 text-white border border-white/20 font-bold text-sm transition flex items-center justify-center gap-2.5 cursor-pointer"
              >
          ...
          <div className="bg-white rounded-[2rem] p-6 sm:p-8 shadow-xl shadow-slate-100 border border-slate-100 space-y-6">
                ...
                {isRegistered ? (
                  isAccessGranted ? (
                    ...
                    <span className="px-3 py-1 rounded-full text-xs font-bold bg-amber-50 text-amber-700 border border-amber-200 flex items-center gap-1.5">
                      <Clock className="w-3.5 h-3.5" /> Enrolled (Pending Approval)
                    </span>
                  )
                ) : (
                  <span className="px-3 py-1 rounded-full text-xs font-bold bg-slate-100 text-slate-700 border border-slate-200 flex items-center gap-1.5">
                    <AlertCircle className="w-3.5 h-3.5 text-amber-500" /> Not Registered
                  </span>
                )}
              </div>
            </div>

            {/* If unregistered banner notice */}
            {!isRegistered && (
              <div className="p-4 rounded-2xl bg-amber-50/80 border border-amber-200/80 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-start gap-3">
                  <AlertCircle className="w-5 h-5 text-amber-700 shrink-0 mt-0.5" />
                  <div>
                    <h4 className="text-xs font-bold text-amber-900">You are not registered for the AI for Educators Bootcamp</h4>
                    <p className="text-[11px] text-amber-800 mt-0.5">
                      Registration is required to access the live Google Meet masterclasses, download curriculum materials, and earn your official accredited certificate.
                    </p>
                  </div>
                </div>
                <Link
                  to="/events/ai-for-educators"
                  className="px-4 py-2 bg-amber-500 hover:bg-amber-400 text-slate-950 rounded-xl font-bold text-xs whitespace-nowrap text-center shrink-0 shadow-sm"
                >
                  Register Now ($10)
                </Link>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                ...
                <p className="text-[11px] text-slate-500">Requires Registration & Admin Approval</p>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-r from-indigo-900 to-indigo-800 rounded-2xl p-5 text-white flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-indigo-200">
                    {isAccessGranted ? (
                      ...
                      <span className="text-amber-300/90 font-medium">Stage link available upon verified registration approval</span>
                    )}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto">
                {isAccessGranted ? (
                  ...
                  <div className="w-full sm:w-auto px-4 py-2 rounded-xl bg-white/10 text-amber-300 font-bold text-xs flex items-center justify-center gap-1.5 border border-amber-400/20">
            ...
            <div className="p-5 rounded-2xl bg-gradient-to-br from-amber-50/70 via-white to-amber-50/40 border border-amber-200/80 space-y-4">
              <div className="flex items-center justify-between">
                ...
                {!isRegistered ? (
                  ...
                  <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-amber-100 text-amber-800 border border-amber-200 flex items-center gap-1">
                    <Lock className="w-3.5 h-3.5" /> Admin Approval Pending
                  </span>
                )}
              </div>
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs font-mono font-bold text-slate-900 bg-white p-2 rounded-xl border border-slate-200/60 break-all">
                  {!isRegistered 
                    ...
                    : isCertUnlocked 
                      ? (certificateData?.certificate_number || registration?.certificate_number || 'PENDING')
              ...
              {!isRegistered ? (
                <div className="space-y-2">
                  <div className="w-full py-2.5 bg-amber-50/80 border border-amber-200 text-amber-900 rounded-xl font-bold text-xs flex items-center justify-center gap-2 text-center px-3 select-none">
                    <Lock className="w-4 h-4 text-amber-600 shrink-0" />
                    <span>Registration required for certificate issuance</span>
                  </div>
                  <Link
                    to="/events/ai-for-educators"
